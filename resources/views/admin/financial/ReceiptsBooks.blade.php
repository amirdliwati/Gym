...
@endsection
@section('breadcrumb')
    <h3>{{__('Reports & Archive Receipts Books')}}</h3>
    <ol class="breadcrumb">
        ...
        <div class="row">
            <table id="receipts-books-table" class="table table-bordered table-hover table-striped dt-responsive" style="border-collapse: collapse; border-spacing: 3; width: 100%;">
                ...
                <tr style="text-align: center;">
                    ...
                    <th class="text-primary">{{__('Archive')}}</th>
                </tr>
                </thead>
                <tbody style="text-align: center;">
                    ...
                    <tr style="text-align: center;">
                        ...
                        @if($Receipt->type == 1)
                            <td><span class="badge badge-primary">Receipt</span> </td>
                        @else
                            <td><span class="badge badge-warning">Payment Receipt</span> </td>
                        @endif
                        ...
                        @switch($Receipt->status)
                            @case(1)
                                <td>
                                    <div class="media-body icon-state">
                                        <label class="switch">
                                            <input type="checkbox" id="statusReceipt{{$Receipt->id}}" checked onclick="Status('{{$Receipt->id}}')"><span class="switch-state"></span>
                                        </label>
                                    </div>
                                </td>
                            @break
                            ...
                            @case(2)
                                <td>
                                    <div class="media-body icon-state">
                                        <label class="switch">
                                            <input type="checkbox" id="statusReceipt{{$Receipt->id}}" onclick="Status('{{$Receipt->id}}')"><span class="switch-state"></span>
                                        </label>
                                    </div>
                                </td>
                            @break
...
@section('javascript')
                ...
                "columnDefs": [{"targets": [ 0 ],"visible": false},
                            ...
                            {"targets": [ 10 ],"render": function ( data, type, row, meta ) {
                                    switch (data["status_receipt"]) {
                                        case 1:
                                            return '<div class="media-body icon-state"><label class="switch"><input type="checkbox" id="statusReceipt'+data["id_receipt"]+'" checked onclick="Status('+data["id_receipt"]+')"><span class="switch-state"></span></label></div>';
                                        break;
                                        case 2:
                                            return '<div class="media-body icon-state"><label class="switch"><input type="checkbox" id="statusReceipt'+data["id_receipt"]+'" onclick="Status('+data["id_receipt"]+')"><span class="switch-state"></span></label></div>';
                                        break;
                                        case 3:
                                            return '<span class="badge badge-danger">Canceled</span>';
                                        break;
    ...
    <!-- Status -->
    <script type="text/javascript">
        function Status(ReceiptID) {
            $.ajax({
                type: "GET",
                url: "{{url('ArchivedReceiptsBooks')}}",
                data: {'ReceiptID': ReceiptID},
                cache:false,
                contentType: false,
                success: function (data) {
                    TableReceiptsBooks(data["ReceiptsBooks"]);
                    if (data["Status"] == 'ok') {
                        NotifyMessage('Success','The receipt has been archived','success');
                    } else {
                        NotifyMessage('Success','The receipt has been not archived','danger');
                    }
                },
                error: function(jqXHR){
                    if(jqXHR.status==0) {
