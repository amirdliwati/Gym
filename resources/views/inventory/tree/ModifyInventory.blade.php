...
@endsection
@section('bookmark')
    <li><button type="button" class="btn btn-outline-warning" data-toggle="modal"  data-target="#modalRename"><i class="mdi mdi-18px mdi-circle-edit-outline mr-2"></i>{{__('Rename')}}</button></li>

    <li><button type="button" class="btn btn-outline-primary" data-toggle="modal"  data-target="#modalChangeParent"><i class="mdi mdi-18px mdi-folder-edit mr-2"></i>{{__('Change Parent')}}</button></li>

    <li><button type="button" id="add-category" class="btn btn-outline-danger" onclick="DeleteCategory()"><i class="mdi mdi-18px mdi-trash-can-outline"></i></button></li>
@endsection
@section('content')
<!-- Main content -->
<div class="card">
    <input type="hidden" id="category_id" name="category_id" value="">
    <div class="card-header b-l-primary border-3"><h5> {{__('Modify Inventory Tree')}} </h5> @include('layouts/button_card') </div>
    <div class="card-body">
        <div class="row">
...
<!-- Large modal Rename Category -->
<form id="form" data-parsley-required-message="">
    @csrf
    <input type="hidden" id="rename_category_id" name="rename_category_id" value="">
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myLargeModalLabel">{{__('Rename Category')}}</h5>
...
<!-- Large modal Change Parent Category -->
<form id="form2" data-parsley-required-message="">
    @csrf
    <input type="hidden" id="change_parent_category_id" name="change_parent_category_id" value="">
    <div class="modal fade bs-example-modal-sm" id="modalChangeParent" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myLargeModalLabel">{{__('Change Parent Category')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                    <div class="row form-material">
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        inventoryTree.on('changed.jstree', function(e, data) {
            ...
            $("#category_id").val(r[0]);
            $("#rename_category_id").val(r[0]);
            $("#change_parent_category_id").val(r[0]);
        });
    ...
    <script type="text/javascript">
        function DeleteCategory () {
            swal({
                ...
                preConfirm: function (){
                    if ($("#category_id").val() == "") {
                        NotifyMessage('Error', 'You did not select category', 'danger');
                    } else {
                        $.ajax({
                            type: "GET",
                            url: "/DeleteCategory/" + $("#category_id").val(),
                            contentType: false,
                            success: function (data) {
                                if (data == 'Delete Error') {
                                    SwalMessage('Delete Error','You can not delete this category becuase it has children.','error');
                                    $("#category_id").val("");
                                } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == 'Rename Error') {
                        SwalMessage('Error','You can not Rename category becuase did not select category.','error');
                    } else {
                        $("#jstree").jstree('destroy');
                        TreeData(data);
                    }
        ...
        $('#form2').on('submit', function(event) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == 'Error') {
                        SwalMessage('Attention','You can not change parent.','warning');
                    } else {
                        $("#jstree").jstree('destroy');
                        TreeData(data);
                    }
